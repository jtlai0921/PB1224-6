...
public class AddParagraphInSection {
	public static void main(String[] args){
		...
		try {
			PdfWriter.getInstance(document, new FileOutputStream("C:\\在小節中增加段落.pdf"));// 關聯文件對象與輸出流
			document.open();// 開啟文件
			...
			paragraph = new Paragraph("小節中增加的內容",fontChinese3);// 建立段落
			section.add(paragraph);// 向小節增加段落內容 
			paragraph = new Paragraph("小節中增加的另一部分內容",fontChinese3);// 建立段落
			section.add(paragraph);// 向小節增加段落內容 
